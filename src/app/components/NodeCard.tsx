...
import { NodeHealthStatus } from "../../services/dashboardService.type";
...
export default function NodeCard({ node }: NodeCardProps) {
  ...
  const dbResponseData: any = [];

  const isCritical = node.health_status === "CRITICAL";

  return (
    <Card
      className={`bg-white border-l-4 shadow-sm hover:shadow-md transition-all duration-300 ${
        isCritical
          ? "border-l-red-500 node-card-critical"
          : node.status === "warning"
            ? "border-l-amber-500"
            : "border-l-green-500"
    ...
    >
      <CardHeader className="pb-1.5 pt-3 px-3">
        <div className="flex items-start justify-between">
          ...
          <Badge
            className={`${getStatusColor(node.health_status)} border font-semibold px-1.5 py-0.5 text-xs uppercase tracking-wide ${
        ...
        <div className="grid grid-cols-3 gap-1.5 pt-2 border-t border-gray-100">
          <div className="text-center p-1.5 bg-gray-50 rounded">
            ...
            <div className="flex items-center justify-center gap-0.5">
              <Badge
                className={`${getStatusColor(node.amb_status)} border font-semibold px-1.5 py-0.5 text-xs uppercase tracking-wide ${
                  isCritical ? "badge-critical" : ""
                }`}
              >
                {node.amb_status}
              ...
              {/* <p className="text-xs font-bold text-gray-900">
                {Number(node.avg_db_time_ms) > 1
                  ? `${(Number(node.avg_db_time_ms) / 1000).toFixed(1)}k`
                  : Number(node.avg_db_time_ms)}
              </p>
              {getTrendIcon(Number(node.avg_db_time_ms) > 0.7 ? "down" : "up")} */}
            </div>
          </div>
          ...
          >
            ...
            <p
              className={`text-xs font-bold ${
                Number(node.avg_queue_length_ms) > 0.1
                  ? "text-red-600 badge-critical"
                  : "text-gray-900"
              }`}
            >
              {node.avg_queue_length_ms}
            </p>
          </div>

          <div
            className={`text-center p-1.5 rounded ${
              Number(node.slow_transaction_percent) > 40000
